...
const university = getUniversityById("eimt")!;
const dbaDetails = university.dbaDetails!;

export const metadata: Metadata = {
  title: `${university.name} - Doctor of Business Administration (DBA)`,
  description: `${university.description} European DBA with international reach. Explore curriculum, tuition, and apply today.`,
};

export default function EIMTDBAPage() {
  return (
    <main>
      <PageHero
        eyebrow={`DBA · ${university.name}`}
        title={`Doctor of Business Administration`}
        description={`Executive doctorate from ${university.name}. European perspective with global standards.`}
            ...
            <div className="mt-6 grid gap-4">
              <div>
                ...
                <p className="mt-2 text-lg font-semibold text-slate-950">
                  {university.duration?.dba ? `${university.duration.dba.min}-${university.duration.dba.max} months` : "20-40 months"}
                </p>
              </div>
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-400">
      ...
      <section className="bg-white py-24">
        <div className="container-premium">
          <SectionHeading
            ...
            description="Key focus areas and modules in EIMT's DBA programme."
          />
          <div className="mt-14 grid gap-4 max-w-3xl">
